...
import { Header, Banner, CardMotorcycles, Footer } from '../../components'

import formatCurrency from '../../utils/formatCurrency'

import api from '../../services/api'

export function Motorcycles ({ location: { state } }) {
  let brandId = 0
  if (state?.brandId) {
  ...
  useEffect(() => {
    async function loadBrands () {
      ...
      let brandsMotors = data.slice(6, 9)
  ...
  return (
    <Container>
      ...
      <MotorsContainer>
        {filteredMotorcycles &&
          filteredMotorcycles.map(moto => <CardMotorcycles key={moto.id} moto={moto} />)}
      </MotorsContainer>
      <Footer />
    </Container>
  )
